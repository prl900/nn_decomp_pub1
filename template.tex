...
The PCA of a matrix $M$ is typically computed applying singular value decomposition (SVD) to the covariance matrix. The Eckart-Young theorem \citep{eckart1936approximation}, tells us that this decomposition, truncated to the first $n$ factors, is the best rank-n approximation in the least squares sense, amongst the space of all possible decompositions $M$. Although PCA results in the best factorisation of $M$, the solution is not unique and alternative methods can be employed to find alternative decompositions. 

From an optimisation point of view, this decomposition problem can be formulated as:

$$
\arg\:\min_{UV} \mathbf{MSE}(M-UV)
$$

Neural networks can be regarded as a composition of matrix multiplications with applied non-linearities. The resulting models can approximate any function, given a large enough number of layers. In this work we employ neural networks to perform matrix decompositions using gradient descent optimisation. This can be expressed with a single layer neural networks that computes the product of these two matrices, with no non-linear operations involved.

As opposed to PCA, the proposed methodology to decompose a matrix does not introduce any constraints on the factorisation process, and results in a convex optimisation problem. 


Although PCA identifies the global minimum of this optimisation problem, the resulting factorisation is not unique. 

 In our case, this matrix product is optimised to identify the best approximation, in the mean squared error (MSE) sense, to the matrix containing the original temporal series of satellite images -- represented in Figure \ref{dataset_detail} c). In the case of PCA, these two matrices represent the principal components of the original signal and the reconstruction coefficients. 
