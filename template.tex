...
Temporal-based methodologies are best suited for satellite missions with revisit times of a few days \citep{shen2015missing}. This type of time frame ensures that the changes in the surface reflectance signal between any two consecutive images are small but the atmospheric conditions present a large variability. The most common approach for this type of data consists in applying regression or interpolation methods along the temporal dimension to impute the missing values \citep{zeng2013recovering,zhang2014missing,zeng2014reconstructing}. 


2 The extension to multidimensional or tensor approaches

The 4-dimensional nature of time series of multispectral remote sensing data is well suited for tensor decomposition techniques \citep{kolda2009tensor}. These techniques have been explored in the context of hyperspectral remote sensing image compression \citep{zhang2015compression}. Internally, most of the methodologies for decomposing multidimensional arrays use high order singular value decomposition (HOSVD) to perform the factorisations. HOSVD basically consists in reordering or 'unfolding' the tensor data into its matrix form and applying traditional linear algebra algorithms to compute the decomposition of this matrix. Different patterns for unfolding the original tensor into a matrix form lead to large differences in the quality of the decomposition. This is largely dependent on the spectral and spatial correlations in the original tensor and how well these are represented in its matrix form. Regardless of the dimensionality used to represent the data, the factorisation methods use traditional linear algebra algorithms to compute the decomposition. 

factorisation relies on optimisation 



Therefore, the results of PCA decomposition will depend on the chosen missing data imputation process. Regardless of the methodology, the process of imputing missing values introduces artificial, non-observed values in the original signal, leading to errors and biases in the resulting decomposition.

The second disadvantage of PCA is 
