...
Each tile, as previously described, can be represented with a cube formed by stacking 400 by 400 pixel images along the temporal dimension. In time, the number of images ranges between 85 and 130 depending on the position. The data is finally stored stacking each spectral band along the temporal dimension. Figure \ref{dataset_detail} represents the the stacked time series using a true colour representation and Fieach scene s in the spatial dimensions and 

\begin{figure}%
    \centering
    \subfloat[Precision scores]{{\includegraphics[width=8cm]{figure2a.png} }}%
    \quad
    \subfloat[Recall scores]{{\includegraphics[width=8cm]{figure2b.png} }}%
