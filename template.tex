...
PCA of a matrix $M$ is typically computed via the singular value decomposition (SVD). The Eckart-Young theorem \citep{eckart1936approximation}, tells us that this decomposition, truncated to the first $n$ factors, is the best rank-n approximation in the least squares sense, amongst all possible decompositions of $M$. Although PCA achieves the best factorisation of $M$, its solution is not unique and other methods can be employed to find alternative decompositions of the same quality. 

From an optimisation point of view, this factorisation problem can be formulated as:

$$
\arg\:\min_{AB} \mathbf{MSE}(M-AB)
$$

Where $\mathbf{MSE}$ stands for the mean square error and $A$ and $B$ are the factor matrices to optimise. 

In this work, we employ neural networks to solve this optimisation problem using gradient descent optimisation. This can be expressed using a one-layer neural network that computes the product of two matrices of predefined shape. The neural network is then optimised to minimise the distance between the original matrix and the product of the factors $A$ and $B$. This network does not make use of non-linear activations and does not require an input to be trained, so the result is a linear and unsupervised model.

As described in the previous section, we consider $M$ to be the matrix containing temporal series of satellite images, as represented in Figure \ref{dataset_detail} c). We assume that the dynamics of the landscape, expressed through the evolution in time for the different spectral bands, can be captured by a matrix with significantly lower rank than the original. Figure \ref{factorisation} represents the reconstruction of the unbiased satellite observation matrix, $M$, as the product of two matrices, $A$ and $B$. Matrix $M$ in this case contains 138 rows and 160000 columns, corresponding to the temporal dimension and number of pixels (400x400) respectively, in the observation stack. In this representation, the number of components is truncated to 12. Matrix $B$ contains 12 principal components of the same size as the original 400x400 tiles. Matrix $A$ contains the coefficients that multiply each of the components to recreate each of the temporal entries in the original matrix C. The number of columns of $A$ is the same as the rows of $B$, and this number determines the truncation performed to the reconstruction. This number sets therefore a trade-off between the achieved accuracy in the reconstruction of matrix $M$ and the compression ratio with respect to the original image. In this case, considering the total size of the matrices on each side of the equality, we get $(138 \cdot 160000) / ((138 \cdot 12) + (12 \cdot 160000)) = 84 $. 
...
In PCA, we commonly refer to $A$ as the coefficients matrix and $B$ as the principal components matrix. The reconstruction process presented in Figure \ref{factorisation} is the same regardless of the methodology used to compute $A$ and $B$. Figure \ref{factorisation} represents the resulting matrix $B$ computed using PCA and NN for decomposing the observation matrix. The plots on the lower right show $B^TB$ which represents the inner products between each component. In the case of PCA we can see that principal components form an orthonormal basis, showing entries equal to one along the main diagonal and zero in the rest. In the case of the NN factorisation the components are not orthogonal. Another difference is that the principal components are ordered from the top-left to the bottom-right in order of their importance at explaining the variance in the original signal for the case of PCA but not such ordering exists in the case of NN. We acknowledge that the NN methodology generates different factorisations to PCA, however, we intentionally use the same terminology to refer to the factor matrices for stablishing a link between both methodologies and for improving clarity.
...
In the case of PCA, these two matrices represent the principal components of the original signal and the reconstruction coefficients. 

As opposed to PCA, the proposed methodology for decomposing a matrix does not impose any constraints on the solution. Factors $A$ and $B$ do not need to be orthogonal and , and results in a convex optimisation problem. 
