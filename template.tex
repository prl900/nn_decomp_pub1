...
Remote sensing provides valuable information about the Earth at an increasing level of detail in the spatial, spectral and temporal domains. Due to the size of current remote sensing collections, processing requires accessing large amounts of storage and compute resources, making this task difficult and expensive. In the case of studying land processes, only the reflectance component coming from the Earth's surface is used, and all other effects originating in the atmosphere, such as clouds or shadows, can be considered noise contaminating the land's signal. Changes in the land typically happen at lower temporal scales than those in the atmosphere, which suggests that the land dynamics can be described using in much lower dimensional space than the original data. This idea leads to the exploration of methodologies that exploit the inherent redundancies in the data and find efficient representations able to capture the variability in the signal coming just from the land.

From an algebraic perspective, we can say that there exists a low-rank representation of the Earth's surface signal that is able to capture most of the variability in the original signal. Finding this low-rank representation is the objective of Principal Component Analysis (PCA). The use of PCA in remote sensing has been known for many years and it has been extensively explored in the context of noise filtering and the compression of images.

One of the limitations of PCA is its inability to work with missing values. As discussed before, remote sensing images representing the land surface, are often contaminated with atmospheric signals that need to be masked, creating gaps of missing data in the images. There are different approaches for dealing with missing values before applying PCA, such as, temporally aggregating the data, assign fixed values to the missing data or interpolation using close neighbours in space or time. Therefore, the result of a PCA decomposition depends on the missing data imputation process used. In any case, all these methods impute artificial, non-observed values in the original signal, introducing biases.

PCA calculates the principal components as the eigenvectors of the correlation matrix. This implies that principal components form an orthogonal basis for reconstructing the original matrix. This orthogonality constraint ensures that the information expressed by each component is unique and independent from the others. However, from an optimisation point of view, it defines a non-convex problem. Removing the orthogonality constraint makes the problem convex, which allows applying common optimisation techniques, such as gradient descent, to find valid decompositions of the original data. A framework that generalises PCA under different constraints has been proposed \citep{udell2014generalized}. In this work we propose implementing the unconstrained PCA matrix decomposition using neural networks and learn the decomposition using gradient descent. Removing the orthogonality constraint makes the problem convex, so gradient descent should converge converge towards the global minumum of the reconstruction error function.

We use mean square error (MSE) as the objective function that measures the error in the reconstructed matrix. To deal with missing values, we use only for the valid pixels to compute the error. The resulting reconstructed stack of images hand we evaluate A series of experiments are performed to evaluate the and compared to classic PCA with pre-interpolated data.

In this work we explore how neural networks can be used to find efficient decompositions of remote sensing time series data that can deal with missing values. We present a methodology for and test it using Sentinel-2 data over a region in Australia. 
...
original data the variability of the Earth's surface, as captured by remote sensing, can be described using a . This fact has been extensively explored and exploited in the literature to create compressed and remove the noise from satellite images. 
